...
def main():
    ...
    print("📂 Liczenie dokumentów...")
    total_docs = sum(1 for line in open(DATA_PATH, "r", encoding="utf-8") if line.strip())
    print(f"✅ Znaleziono {total_docs:,} dokumentów")
    
    print("🚀 Przetwarzanie strumieniowe...")
    point_id = 0
    total_chunks = 0
    doc_batch = []
    
    with open(DATA_PATH, "r", encoding="utf-8") as f:
        for line_num, line in enumerate(tqdm(f, total=total_docs, desc="Dokumenty")):
            if not line.strip():
                continue
            ...
            if len(doc_batch) >= DOC_BATCH_SIZE:
                point_id, chunks_count = process_batch(doc_batch, model, client, point_id)
    ...
    print(f"\n✅ ZAKOŃCZONO! Zaindeksowano {total_chunks:,} chunków w '{COLLECTION_NAME}'")


def process_batch(documents, model, client, start_point_id):
    """Przetwarza batch dokumentów: chunking → embedding → upload"""
    all_chunks = []
    all_payloads = []
    ...
    return point_id, len(all_chunks)
